...
let uniques sudoku =
    seq { for row in [1..9] do
          ...
          let rowNums = [1..9] |> List.map (fun col -> atPos row col sudoku)
          let colNums = [1..9] |> List.map (fun row -> atPos row col sudoku)
          let sectionNums =
          ...
          let combinedNums = List.concat [rowNums; colNums; List.ofSeq sectionNums] |> List.filter ((<>) x) |> Set.ofList
...
let rec solveEverything sudoku =
    let uniques = uniques sudoku |> Seq.cache
    ...
    else solveEverything (List.ofSeq (replace sudoku uniques))