...
BOOL isPrimitive(LISP_DATA* exp)
{
	return exp->type == STRING 
			&& strlen(exp->data.string) == 1 
			&& contains(PRIMITIVE_CHARS, exp->data.string[0]);
}
...
LISP_DATA* lookup(LISP_DATA* symbol, LISP_DATA* env)
{
	...
	while (currentFrame) {
		if(currentFrame->head->data.list->head->data.string[0] == symbol->data.string[0]) {
...
LISP_DATA* makeClosure(LISP_DATA_LIST* args, LISP_DATA* env)
{
	...
	closureString->type = STRING;
	closureString->data.string = "closure";
...
LISP_DATA* eval(LISP_DATA* exp, LISP_DATA* env)
{
	...
	if (exp->type == LIST) 
	{
		...
		else if (car->type == SYMBOL && !stricmp(car->data.string, "let")) 
		{
		...
		else if (car->type == SYMBOL && !stricmp(car->data.string, "lambda")) 
		{
...
LISP_DATA* apply(LISP_DATA* proc, LISP_DATA_LIST* args)
{
	...
	if (proc->type == LIST 
		...
		&& !stricmp(proc->data.list->head->data.string, "closure")) 
	{
...
void testInterpreter(char* inputString)
{
	...
	AST_NODE* root = parse(tokens, tokenLength);
	LISP_DATA* data = parseTree(root);
	LISP_DATA* evaluated = eval(data, 0);
	printLispData(evaluated);
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	testInterpreter("((lambda () ((((lambda (y) (lambda (x) (lambda (z) (+ x y z)))) 10) 67) 6)) ())");
